...
import (
	...
	"github.com/wield18/all-in-one/ekit/tuple/pair"
)
...
const (
	...
	TypeDigit Type = 1
	// TypeLowerCase 小写字母
	TypeLowerCase Type = 1 << 1 // 位运算左移操作
	// TypeUpperCase 大写字母
...
// length 生成的随机字符多长 typ 生成的随机字符要哪些字符
...
func generate(source string, length, idxBits int) string {
	...
	for i := 0; i < length; {
		...
		//利用掩码获取有效的随机数位
		// 这是**拒绝采样法（Rejection Sampling）**：
		// 1. 生成一个 [0, 15] 的随机数
		// 2. 如果不在 [0, 9] 范围内，就丢弃重试
		// 3. 只接受有效范围内的值
